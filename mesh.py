...
class PropellerMesh:
        ...
        for i in range(chordwise_points - 1):  # Now 'i' represents chordwise direction
            for j in range(spanwise_points - 1):  # Now 'j' represents spanwise direction
                ...
                if self.propeller_geometry.R_tip > 0:
                    ...
                    # Get twist angle at this r/R from the propeller_geometry
                    twist_angle = self.propeller_geometry.pitch_spline(r_R)
                    # print(f"Twist from CSV: {self.propeller_geometry.pitch_spline(r_R)}")
                    # print(f"Twist applied in geometry: {-np.radians(self.propeller_geometry.pitch_spline(r_R))}")
                    # print(f"Twist in mesh (twist_rad): {twist_angle}")
                    twist[panel_index] = twist_angle
                    
                    # Get polar data for this r/R value and store it for each alpha in our range
                    panel_polar_data = {
                        'r/R': r_R,
                        'alpha': [],
                        'cl': [],
                    ...
                    # For each alpha value, get interpolated coefficients
    ...
    def export_panel_data_to_csv(self, quad_propeller_mesh, propeller_key, blade_key, filename):
        """
        ...
        for panel_idx, control_point in blade_data['Control Points'].items():
            ...
            i, j = panel_idx
            cp_x, cp_y, cp_z = control_point
            
            # Get twist angle
            twist = blade_data['Twist'].get(panel_idx, np.nan)
            
            # Get r/R value and example coefficients at α=0°
            coeff_data_0 = self.get_panel_aerodynamic_coefficients(
                quad_propeller_mesh, propeller_key, blade_key, panel_idx, 0)
            
            # Get coefficients at α=5°
            coeff_data_5 = self.get_panel_aerodynamic_coefficients(
                quad_propeller_mesh, propeller_key, blade_key, panel_idx, 5)
            ...
            row = {
                ...
                'CP_Y': cp_y,
                'CP_Z': cp_z,
                'r/R': coeff_data_0['r/R'],
                'Twist_deg': twist,
                'Cl_alpha0': coeff_data_0['cl'],
                'Cd_alpha0': coeff_data_0['cd'],
                'Cm_alpha0': coeff_data_0['cm'],
                'Cl_alpha5': coeff_data_5['cl'],
                'Cd_alpha5': coeff_data_5['cd'],
                'Cm_alpha5': coeff_data_5['cm'],
            }
        ...
        df.sort_values(by=['Panel_j', 'Panel_i'], inplace=True)  # Sort by j then i
