...
def biot_savart(r1, r2, r0, gamma, result): 
    ...
    if norm_cross < 1e-8 or norm_r0 < 1e-8:
        for i in range(3):
            result[i] = 0.0
        return
    ...
    rc = rc0
    n = 2
    test = -eps * (norm_cross / (norm_r0 * rc))**2

    test2 = 1 - 2.71828**test
    ...
    factor =  (1.0 / (4 * math.pi)) * dot_term / denominator
...
@cuda.jit(device=True)
def biot_savart_wake(r1, r2, r0, dt, step_diff, gamma, current_step, result): 
    ...
    # Compute final velocity
    factor =  Kv * (1.0 / (4.0 * math.pi)) * dot_term / denominator
    result[0] = factor * cross_r1_r2[0]
    result[1] = factor * cross_r1_r2[1]
    result[2] = factor * cross_r1_r2[2]
...
@cuda.jit
def wake_velocity_kernel(control_points, wake_vortex_rings, wake_gamma, wake_time_steps, current_step, dt, result_matrix):
    ...
    if i < control_points.shape[0] and j < wake_vortex_rings.shape[0]:
        ...
        for k in range(4):
            ...
            # biot_savart_wake(r1, r2, r0, dt, step_diff, wake_gamma[j], current_step, induced)
...
@cuda.jit
def bound_to_wake_velocity_kernel(wake_vertices, bound_vortex_rings, result_matrix, wake_gamma, wake_time_steps, dt, current_step):
...
def wake_to_wake_velocity_kernel(wake_vertices, wake_vortex_rings, result_matrix, wake_gamma, wake_time_steps, current_step, dt):
    ...
    if i < wake_vertices.shape[0] and j < wake_vortex_rings.shape[0]:
        ...
        for k in range(4):
            ...
            biot_savart_wake(r1, r2, r0, dt, step_diff, gamma_value, current_step, induced)
...
class UVLM:
    ...
    def calculate_wake_to_bound_induced_velocity_matrix(self, quad_propeller_mesh, dt, current_step):
        ...
        for propeller_key, propeller_data in quad_propeller_mesh.items():
            ...
            wake_velocity_kernel[blockspergrid, threadsperblock](
                ...
                d_wake_time_steps, current_step, dt, d_result_matrix
            )
            ...
            induced_velocities = np.einsum('ijk,j->ik', result_matrix, wake_gamma.flatten())
        ...
        for propeller_key, propeller_data in quad_propeller_mesh.items():
            ...
            for blade_key, blade_data in propeller_data['Blades'].items():
                ...
                for panel_index, control_point in control_points.items():
                    ...
                    # gamma_previous_span = blade_data['Gamma'].get((panel_index[0] + 1, panel_index[1])) if panel_index[0] > 0 else 0
    ...
    def update_wake(self, propeller_mesh, time_step, dt, body_velocity, omega_dict, wind_field, com_position, vpm):
        """
        ...
        """
        MAX_WAKE_LENGTH = 200 #Maximum number of spanwise panels to maintain
        wind_func = WindField.update_wind_function(wind_field, com_position)
                    ...
                    if panel_index[0] == max_spanwise_index:
                        ...
                        if time_step == 1:
                            ...
                            new_vertices = np.array([
                                ...
                                te_left + 0.25 * (wake_vertices_0[0] - te_left),     # 30% of the way to previous front left
                                te_right + 0.25 * (wake_vertices_0[3] - te_right),   # 30% of the way to previous front right
                            ...
                            panel_coordinates = (0, chordwise_idx)
                            previous_chord_coordinates = (panel_coordinates[0], panel_coordinates[1] - 1)
                            next_span_coordinates = (panel_coordinates[0] + 1, panel_coordinates[1])

                            gamma = wake_panel_0['Gamma']
                            gamma_previous_chord = self.wake_system[propeller_key][blade_key].get(previous_chord_coordinates, {}).get('Gamma', 0) if panel_coordinates[1] > 0 else 0
                            gamma_after_span = self.wake_system[propeller_key][blade_key].get(next_span_coordinates, {}).get('Gamma', 0)
            
                            # 2. Create particles for left side filament
                            if panel_coordinates[1] == max_chordwise_index:
                                # 3. Create particles for right side filament
                                vector_right = new_vertices[1] - new_vertices[0]  # Back right - Front right
                                vpm.create_vortex_particles_from_line(
                                    propeller_key,
                                    blade_key,
                                    new_vertices[0],  # Start point (front right)
                                    new_vertices[1],  # End point (back right)
                                    gamma - gamma_previous_chord,  # Circulation
                                    vector_right,  # Direction vector along right edge
                                    time_step
                                )
                
                                vector_right = new_vertices[2] - new_vertices[3]  # Back right - Front right
                                vpm.create_vortex_particles_from_line(
                                    propeller_key,
                                    blade_key,
                                    new_vertices[3],  # Start point (front right)
                                    new_vertices[2],  # End point (back right)
                                    gamma,  # Circulation
                                    vector_right,  # Direction vector along right edge
                                    time_step
                                )
                            ...
                            # 4. Create particles for back filament (between back left and back right)
                            vector_back = new_vertices[2] - new_vertices[1]  # Back right - Back left
                            # print('gamma', gamma_after_span - gamma)
                            vpm.create_vortex_particles_from_line(
                                propeller_key,
                                blade_key,
                                new_vertices[1],  # Start point (back left)
                                new_vertices[2],  # End point (back right)
                                gamma_after_span - gamma,  # Circulation
                                vector_back,  # Direction vector along back edge
                                time_step
                            )
                            
    def update_wake_abm(self, propeller_mesh, time_step, dt, body_velocity, omega_dict, wind_field, com_position):
        """
        Update wake system with a maximum of 10 spanwise panels by:
        1. First convecting existing wake panels
        2. Then shedding new wake panels
        ...
        if time_step >= 3:
            for propeller_key, wake_data in self.wake_system.items():
                ...
                if not hasattr(self, 'wake_history'):
                    self.wake_history = {}
            
                if propeller_key not in self.wake_history:
                    self.wake_history[propeller_key] = {}
                ...
                for blade_key, blade_data in wake_data.items():
                    if blade_key not in self.wake_history[propeller_key]:
                        self.wake_history[propeller_key][blade_key] = {}
                    for wake_idx, wake_panel in blade_data.items(): # Create list to avoid runtime modification issues
                        ...
                        # Ensure we have history for this wake panel
                        panel_key = f"{wake_idx[0]}_{wake_idx[1]}"
                        if panel_key not in self.wake_history[propeller_key][blade_key]:
                            self.wake_history[propeller_key][blade_key][panel_key] = []
                        
                        panel_history = self.wake_history[propeller_key][blade_key][panel_key]
                        ...
                        for k in range(4):
                            ...
                            current_velocities.append(vertex_velocity)
                        ...
                        if len(panel_history) > 3:
                            panel_history.pop(0)
                    
                        # If we have enough history, use ABM
                        if len(panel_history) == 3:
                            # Predictor step (Adams-Bashforth)
                            for k in range(4):
                                # Get velocities from history
                                v_n = panel_history[0]['velocities'][k]
                                v_n1 = panel_history[1]['velocities'][k]
                                v_n2 = panel_history[2]['velocities'][k]
                                
                                # Predict new position
                                r_n1 = panel_history[2]['positions'][k]
                                predicted_pos = r_n1 + (dt/12) * (23*v_n2 - 16*v_n1 + 5*v_n)
                                
                                # Store predicted position
                                wake_panel['Vortex Rings']['Vertices'][k] = predicted_pos
                            
                            # Calculate velocities at predicted positions
                            predicted_velocities = self.calculate_velocities_at_positions(wake_panel['Vortex Rings']['Vertices'], propeller_mesh, dt, time_step)
                            
                            # Corrector step (Adams-Moulton)
                            for k in range(4):
                                v_pred = predicted_velocities[k]
                                v_n = panel_history[0]['velocities'][k]
                                v_n1 = panel_history[1]['velocities'][k]
                                ...
                                corrected_pos = r_n1 + (dt/12) * (5*v_pred + 8*v_n1 - v_n)
                                
                                # Update position
                                wake_panel['Vortex Rings']['Vertices'][k] = corrected_pos
