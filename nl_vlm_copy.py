...
class VLM:
    ...
    def viscous_coupling(self, quad_propeller_mesh, omega_dict, 
                        ...
                        rho=1.225, relaxation_factor=0.09, max_iterations=50, convergence_tolerance=1e-4):
            """
            ...
            for propeller_key, propeller_data in quad_propeller_mesh.items():
                ...
                for blade_key, blade_data in propeller_data['Blades'].items():
                    ...
                    if 'lambda_ratio' not in blade_data:
                        blade_data['Gamma_avg'] = {}  # To store average circulation for each spanwise section
                        blade_data['lambda_ratio'] = {} 
                        blade_data['Gtable'] = {}  # To store strength ratios
                    ...
                    for spanwise_idx, cp_indices in spanwise_sections.items():
                        ...
                        for iteration in range(max_iterations):
                            ...
                            if iteration == 0:
                                ...
                                twist_rad = np.radians(blade_data['Twist'][rep_idx])
                                ...
                                gamma_avg = blade_data['Gamma_avg'][spanwise_idx]
                                lambda_ratio = blade_data['lambda_ratio'][rep_idx]
                                gamma_old = gamma_avg
                                gamma_table = gamma_avg

                                vertices = blade_data['Vortex Rings'][rep_idx]['Vertices']
                                control_point = blade_data['Control Points'][rep_idx]
                                total_induced_velocity = 0.0
                            ...
                            # Process each filament of the vortex ring
                                for k in range(4):
                                    ...
                                    # Calculate induced velocity contribution
                                    induced_velocity_gamma = self.biot_savart(
                                        r1, r2, r0, gamma=1.0
                                    )
                                    total_induced_velocity += induced_velocity_gamma


                                # alpha_local = alpha_3d + delta_alpha
                            delta_gamma = gamma_table - gamma_old
                            delta_induced_velocity = total_induced_velocity * delta_gamma
                            induced_velocity = induced_velocity + delta_induced_velocity

                            # induced_velocity = induced_velocity + (total_induced_velocity  * (gamma_table - gamma_old))
                            inflow_velocity = -rotational_velocity + induced_velocity + wake_induced_velocity

                            # inflow_velocity = -rotational_velocity + induced_velocity + wake_induced_velocity
                            inflow_angle = np.arctan2(np.abs(np.dot(inflow_velocity, propeller_axis)), np.abs(np.dot(inflow_velocity, tangential_dir)))
                            
                            alpha_eff = inflow_angle
                            # 
                            ...
                            total_force = rho * gamma_table * (np.cross(inflow_velocity, section_length))
                            ...
                            cl_inv = 2 * np.linalg.norm(total_lift) / (rho * (V_mag**2) *  section_area)
                            ...
                            inflow_velocity_squared = (np.dot(inflow_velocity,a1)**2)  + (np.dot(inflow_velocity,a3)**2)
                            cross_velocity = np.cross(inflow_velocity, section_length)
                            denominator = np.sqrt( ((np.dot(cross_velocity,a1))**2) + ((np.dot(cross_velocity,a3))**2) )
                            gamma_table = 0.5 * inflow_velocity_squared * cl_vis * section_area / denominator
                            print('gamma table', gamma_table)

                            gamma_diff = abs(gamma_table - gamma_avg) / abs(gamma_avg + 1e-8) 
                            # Check convergence for this section
                            if gamma_diff > convergence_tolerance:
                                converged = False
                            ...
                            # delta_alpha += relaxation_factor * ((cl_inv - cl_vis) / (2 * np.pi))
                            gamma_place = gamma_table
                            if iteration == 0:
                                gamma_table = gamma_avg + relaxation_factor * (gamma_table - gamma_old)
                            else:
                                gamma_table = gamma_table + relaxation_factor * (gamma_table - gamma_old)

                            gamma_old = gamma_place


                        for cp_index in cp_indices:
                            lambda_ratio = blade_data['lambda_ratio'][cp_index]
                            blade_data['Gamma'][cp_index] = gamma_table * lambda_ratio
